...
class MessageController(Controller):
    ...
    @get("/")
    async def list_messages(self, request: Request, ticket_id: int) -> List[MessageResponse]:
        ...
        async with async_session_factory() as session:
            ...
            stmt = select(Message).where(Message.ticket_id == ticket_id).order_by(Message.created_at.asc())
            result = await session.execute(stmt)
            messages = result.scalars().all()

            # Filter out whispers for customer
            if current_user.role == "customer":
                messages = [m for m in messages if m.message_type != "whisper"]

            return [message_to_response(m) for m in messages]

    @post("/")
    async def create_message(self, request: Request, ticket_id: int, data: MessageCreate) -> MessageResponse:
        current_user = await get_current_user_from_request(request)
        if not current_user:
            raise NotAuthorizedException("Authentication required.")
...
class UploadController(Controller):
    ...
    @post("/", media_type="application/json")
